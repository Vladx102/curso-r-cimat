---
title: "Capítulo 3 — Importación de datos y verbos de dplyr"
...
El repositorio del curso incluye datasets reales en `data/` para practicar justo esto. Ojo: al *knitear* un `.Rmd`, el working directory de cada chunk es la carpeta donde vive el `.Rmd` (aquí, `docs/capitulos_rmd/`), no la raíz del proyecto — por eso la ruta sube dos niveles:
...
9. Importa `data/empleados.csv` con `read_csv("../../data/empleados.csv")`. Calcula el salario promedio por departamento y cuántos empleados no tienen bono registrado (`NA`).
10. Importa `data/prestamos.csv` (`read_csv("../../data/prestamos.csv")`). Limpia `usuario` con stringr (espacios y mayúsculas inconsistentes) y filtra los préstamos que siguen abiertos (`fecha_devolucion` vacía). ¿Cuántos días lleva prestado el más antiguo?
